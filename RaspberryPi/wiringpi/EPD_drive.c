...
void Dis_Progress(unsigned char progress_len)
{
	...
	EPD_Dis_Part(xDot-xDot/16-5,xDot-xDot/16+4,(yDot-16*progress_len)/2-1,(yDot-16*progress_len)/2-1+16*progress_len,(unsigned char *)buffer,1);
	//2.Load progress bar
	y =0;
	for(z= 0;z < progress_len;z++){
...
Remarks:
********************************************************************************/
void Dis_showtime(unsigned int hour,unsigned int minute,unsigned int sec)
{
	int x,z;
	char temp;
	char len = 16;
